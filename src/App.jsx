  ...
  },
  {
    ...
    topics: [
      ...
      },
      {
        ...
        tips: ["Every flowchart must have exactly ONE start and at least ONE stop.","A diamond always has two exits: YES and NO (or True/False).","Flow lines should never cross if you can help it — reroute to keep it readable.","For loops: the arrow loops BACK to the decision diamond; the 'No' branch exits the loop."],
      ...
      },
      {
        ...
        tips: ["The iteration variable (e.g., n=n−1) MUST change inside the loop, or it runs forever.","break is like an emergency exit — useful for 'search until found' patterns.","continue is like 'skip this one' — useful for filtering out unwanted inputs.","while True: is an intentional infinite loop — always pair it with a break statement."],
      ...
      },
      {
        ...
        tips: ["High-level languages are machine independent — the same code runs on different hardware.","A compiler is faster at runtime; an interpreter is easier for debugging.","In C: #define LENGTH 10 means every occurrence of LENGTH in code becomes 10.","C was created by Dennis Ritchie to develop the UNIX operating system."],
      ...
      },
      {
        ...
        tips: ["Variables can change value during execution; their data TYPE cannot change.","Float literals: must have a decimal point OR an exponent (e), or both.","Invalid float: '2' (no decimal or exponent); '1,000.0' (comma not allowed).","String literals in double quotes \"\"; character constants in single quotes 'a'."],
...
export default function App() {
  ...
  const cc = CARD_CONFIG[card.type];
  const key = `${ci}-${ti}`;
  ...
  const mark = (level) => {
    ...
    if (ti + 1 < course.topics.length) navTo(ci, ti + 1);
    else setMode("topics");
  };
      ...
      <div style={{ position: "sticky", top: 0, zIndex: 20, background: "#0a0d14", borderBottom: "1px solid #151e30" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 4, padding: "10px 14px 0", overflowX: "auto" }}>
          {COURSES.map((c, i) => (
            <button key={i} onClick={() => navTo(i, 0)} style={{
              ...
              background: ci === i ? c.color : "#131c2e",
              color: ci === i ? "#07090f" : "#4b5980",
              flexShrink: 0,
            }}>{c.name}</button>
          ))}
        ...
        <div style={{ padding: "6px 14px 8px" }}>
          <div style={{ fontSize: 13, color: course.color, fontWeight: "bold", marginBottom: 6, lineHeight: 1.3 }}>
            {ti + 1}/{course.topics.length} · {topic.title}
          </div>
          <div style={{ display: "flex", gap: 4, flexWrap: "wrap" }}>
            {course.topics.map((_, i) => {
              const k = `${ci}-${i}`;
              return (
                <div key={i} onClick={() => navTo(ci, i)} title={course.topics[i].title} style={{
            ...
            <div style={{ display: "flex", gap: 3, marginBottom: 14 }}>
              {cards.map((c, i) => (
                <div key={i} onClick={() => setCardIdx(i)} style={{
                  ...
                  background: CARD_CONFIG[c.type].color,
        ...
        {!isChatOpen ? (
          <button
            ...
            style={{
              ...
              boxShadow: "0 4px 20px #38bdf855",
              display: "flex", alignItems: "center", justifyContent: "center",
            }}
          >🤖</button>
        ) : (
          <div style={{
            width: 320, height: 460, background: "#0d1117", borderRadius: 20,
            ...
            <div style={{
              ...
              background: "#0a0d14", borderRadius: "20px 20px 0 0",
            }}>
              <div>
                <div style={{ fontSize: 13, fontWeight: "bold", color: "#38bdf8" }}>Paradigm CI</div>
                <div style={{ fontSize: 10, color: "#3a4f70" }}>{course.name} · {topic.title}</div>
              </div>
              <button
                onClick={() => setIsChatOpen(false)}
                style={{ background: "none", border: "none", cursor: "pointer", fontSize: 16, color: "#4b5980" }}
              >✕</button>
            </div>
            ...
            <div style={{ flex: 1, overflowY: "auto", padding: "10px 12px", display: "flex", flexDirection: "column", gap: 8 }}>
              {chatHistory.length === 0 && (
                <div style={{ fontSize: 12, color: "#3a4f70", textAlign: "center", marginTop: 20, lineHeight: 1.7 }}>
                  ...
                  <span style={{ color: "#38bdf8" }}>{topic.title}</span>
                </div>
              )}
              {chatHistory.map((msg, i) => (
                <div key={i} style={{
                  ...
                  <div style={{
                    ...
                    color: msg.role === "user" ? "#07090f" : "#c8d8ee",
              ...
              {paradigmLoading && (
                <div style={{ alignSelf: "flex-start" }}>
                  <div style={{ padding: "8px 12px", borderRadius: "14px 14px 14px 4px", background: "#1e2a3a", fontSize: 13, color: "#4b5980" }}>
                    thinking...
                  </div>
                </div>
              )}
            ...
            <div style={{ padding: "10px 12px", borderTop: "1px solid #1e2a3a", display: "flex", gap: 8 }}>
              <input
                value={paradigmInput}
                onChange={e => setParadigmInput(e.target.value)}
                onKeyDown={e => {
                  if (e.key === "Enter" && !e.shiftKey && paradigmInput.trim()) {
                    askParadigm(paradigmInput);
                    setParadigmInput("");
                  }
                }}
              ...
              />
              <button
                onClick={() => { if (paradigmInput.trim()) { askParadigm(paradigmInput); setParadigmInput(""); } }}
                style={{
                  padding: "9px 14px", background: "#38bdf8", border: "none",
                  borderRadius: 10, color: "#07090f", fontSize: 13,
